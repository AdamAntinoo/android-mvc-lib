...
//	}
	...
	/**
	 * Sdd a new fragment at the end of the list of already managed fragments. After the addition we notify the Adapter that the
	 * source data has changed so it can update any UI element affected by this change like the indicator is the number of pages
	 * is greater than one.
...
//	}
	...
	/**
	 * This Adapter method goes to the underlying data structure and gets the item identified by the unique id received as a
	 * parameter. In out case of a simple list we should check that this identifier is not out of bounds of the data array. In
	 * such a case we create an empty Fragment to replace the missing item.
